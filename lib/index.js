...
function mergeObj(a, b, ignoreEmpty, excludeMap, noOverwrite, excludeIfExistsMap) {
    if (!a || !b) return a || b;
    ...
    Object.keys(b).forEach(function (bk) {
        if (excludeIfExistsMap && excludeIfExistsMap[bk] && typeof a[bk] !== 'undefined') {

        } else if (excludeMap && excludeMap[bk]) {
...
module.exports = function (input, outputPath, currentOnly, fn) {
    ...
    function cloneChildren(target, path, trigger) {
        ...
        if (current) {
          ...
          mergeObj(target.children, current.children, false, exclude, true, excludeIfExists);
        ...
        if (node.doclet && node.doclet.extends && node.doclet.extends.length > 0) {
            ...
            node.doclet.extends
                ...
                if (parent && parent.length > 0) {
                    ...
                    cloneChildren(node, parent, fullname, exclude, excludeIfExists);
                }
            });
        }
