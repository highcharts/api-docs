...
function getSearchBoost(optionPath) {
    ...
    return (
        manualSearchBoost[optionPath] ||
        (Math.round((
            (optionSpaceCount >= 10 ?
                0 : ((10 - optionSpaceCount) / 20)) +
            (optionPathLength >= 100 ?
                0 : ((100 - optionPathLength) / 200))
        ) * 100) / 100)
    );
...
function toSitemap(product, flatListKeys) {
    ...
    for (var i = 0, ie = flatListKeys.length; i < ie; ++i) {
        option = flatListKeys[i];
        sitemap.push('<url><loc>https://api.highcharts.com/');
        sitemap.push(product + '/' + option);
        sitemap.push('</loc><priority>');
        sitemap.push(getSearchBoost(option));
...
module.exports = function (input, outputPath, currentOnly, fn) {
    ...
    function generateDetails(name, node, opath, product, version, toc, constr) {
        ...
        if (node.children) {
            ...
            templates.dump('main', (opath || outputPath) + filename, {
                ...
                searchBoost: getSearchBoost(node.meta.fullname),
    ...
    templates.load(function () {
                ...
                .map(function (version) {
                    ...
                    .then(function () {
                        ...
                        .then(function () {
                            ...
                            fs.writeFileSync(
                                op + 'sitemap.xml',
                                toSitemap(product, Object.keys(flatList).sort())
                            );
        ...
        .then(() => {
            console.log(('Created sitemaps').green);
        })
