...
module.exports = function (input, outputPath, currentOnly, fn) {
    ...
    function transform(name, node, parentName, parent) {
       ...
       // }
        ...
        if (node.meta.default !== undefined/* && !node.doclet.defaultvalue*/) {
        ...
        if (node.doclet && !node.doclet.type) {
            if (isBool(node.doclet.defaultvalue)) {
                node.doclet.type = {
                    names: ['Boolean']
                }
            }
            if (isStr(node.doclet.defaultvalue)) {
                node.doclet.type = {
                    names: ['String']
                }
            }
            if (isNum(node.doclet.defaultvalue)) {
                node.doclet.type = {
                    names: ['Number']
                }
            }
