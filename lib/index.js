...
module.exports = function (input, outputPath, currentOnly, options, fn) {
    var versionFuns = [],
        commonSeries = {},
        ...
        platform = isStr(options.platform) ? options.platform : '',
        includeProducts = isObj(options.products) ? options.products : {},
    ...
    function transform(name, node, parentName, parent) {
        ...
        if (node.doclet && node.doclet.productdesc) {
            node.doclet.productdesc.value = marked(node.doclet.productdesc.value);
        }
