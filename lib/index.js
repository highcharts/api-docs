...
function toSitemap(product, flatListKeys) {
    ...
    for (var i = 0, ie = flatListKeys.length; i < ie; ++i) {
        ...
        sitemap.push(product + '/' + option + '.html');
        sitemap.push('</loc><priority>');
        if (typeof manualBoost[option] !== 'undefined') {
            sitemap.push(manualBoost[option]);
        }
        else {
            sitemap.push(Math.round((
                ((11 - option.split('.').length) / 20) +
                ((100 - option.length) / 200))
            * 100) / 100);
        }
