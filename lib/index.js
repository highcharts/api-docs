...
module.exports = function (input, outputPath, currentOnly, fn) {
    ...
    function transform(name, node, parentName, parent) {
        ...
        if (node.doclet && node.doclet.type) {
            ...
            node.doclet.type.names.forEach(function (t) {
                if (t.trim().indexOf('array') === 0) {
                    node.doclet.types['array'] = extractArrayType(t);
                } else {
    ...
    function dumpNav(node, opath) {
        opath += 'nav/';

        if (node.children && node.children.length > 0) {
            fs.writeFileSync(
                opath + node.meta.fullname + '.json',
                JSON.stringify(node.children.map(function (child) {
                    return {
    ...
    function generateDetails(name, node, opath, product, version, toc) {
