...
module.exports = function (input, outputPath, currentOnly, fn) {
    ...
    /**
     * Return the value if the {value}_products object is undefined or includes the
     * product.
     */
    function productFilter(doclet, key, product) {
        var value = doclet[key];
        products = doclet[key + '_products'];
        if (!value || !products || products.indexOf(product) > -1) {
            return value;
        }
    }
